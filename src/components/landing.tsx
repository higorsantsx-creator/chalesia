...
const CatalogCard = ({ project, onClick, isFaded }: { project: ProjectModel; onClick: () => void; isFaded: boolean }) => {
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className="group relative cursor-pointer bg-card border border-border/5 rounded-sm overflow-hidden shadow-2xl transition-all duration-700 hover:-translate-y-4"
    >
      <div className="aspect-[4/5] overflow-hidden relative">
        <motion.img
          ...
          className="w-full h-full object-cover grayscale-[0.2] group-hover:grayscale-0 group-hover:contrast-125 transition-all duration-700"
        />
        ...
        <div className="absolute top-6 left-6 flex gap-2">
          <span className="text-[8px] uppercase tracking-[0.3em] font-bold bg-primary/90 text-primary-foreground px-4 py-2 backdrop-blur-md rounded-full shadow-lg">
            {project.category}
          </span>
        </div>

        <div className="absolute bottom-0 left-0 w-full p-8 translate-y-8 group-hover:translate-y-0 transition-transform duration-700">
          <div className="flex items-center gap-3 text-white/60 text-[8px] uppercase tracking-[0.2em] font-bold mb-3 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
            <span className="flex items-center gap-1"><Square size={10} /> {project.size}</span>
            <span className="w-1 h-1 rounded-full bg-primary/40" />
            <span className="flex items-center gap-1"><Bed size={10} /> {project.bedrooms} Quartos</span>
          </div>
          
          <h3 className="text-white text-2xl md:text-3xl font-serif font-bold tracking-tight mb-4 group-hover:text-primary transition-colors duration-500">
            {project.name}
          </h3>
          
          <p className="text-white/70 text-sm font-serif italic mb-8 max-w-xs opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-200 line-clamp-2">
            "{project.description}"
          </p>
          ...
          <div className="overflow-hidden">
            <motion.div 
              ...
              className="flex items-center gap-3 text-primary text-[9px] uppercase tracking-[0.3em] font-bold"
            >
...
const ProjectModal = ({ 
  ...
  isOpen: boolean; 
  onClose: () => void;
  onCustomize: (projectName: string) => void;
}) => {
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-12">
          ...
          <motion.div
            ...
            className="relative w-full h-full md:max-h-[90vh] bg-card overflow-hidden shadow-[0_50px_100px_rgba(0,0,0,0.3)] md:rounded-sm flex flex-col md:flex-row border border-border/10"
          >
            <button 
              onClick={onClose}
              className="absolute top-8 right-8 z-[110] text-foreground/50 hover:text-foreground p-3 bg-background/20 backdrop-blur-md rounded-full border border-border/10 transition-all hover:rotate-90"
            >
              <X size={24} strokeWidth={1.5} />
            </button>

            {/* Gallery Section */}
            <div className="w-full md:w-[60%] h-[50vh] md:h-full relative group bg-black">
              ...
              <div className="absolute inset-0 flex items-center justify-between px-6 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                <button 
                  onClick={() => setActiveImg((prev) => (prev - 1 + project.gallery.length) % project.gallery.length)}
                  className="p-4 bg-background/20 backdrop-blur-md border border-border/10 rounded-full text-white hover:bg-primary transition-all shadow-xl"
                >
                  <ChevronLeft size={24} />
                </button>
                <button 
                  onClick={() => setActiveImg((prev) => (prev + 1) % project.gallery.length)}
                  className="p-4 bg-background/20 backdrop-blur-md border border-border/10 rounded-full text-white hover:bg-primary transition-all shadow-xl"
                >
              ...
              <div className="absolute bottom-8 left-8 right-8 flex gap-3 overflow-x-auto pb-4 hide-scrollbar">
                {project.gallery.map((img, idx) => (
                  <button 
                    key={idx}
                    onClick={() => setActiveImg(idx)}
                    className={cn(
                      "w-20 aspect-video rounded-sm overflow-hidden border-2 transition-all shrink-0 shadow-lg",
                      activeImg === idx ? "border-primary scale-105" : "border-transparent opacity-50 grayscale hover:opacity-100 hover:grayscale-0"
                    )}
                  >
            ...
            {/* Content Section */}
            <div className="flex-1 p-8 md:p-16 overflow-y-auto custom-scrollbar bg-card">
              <motion.div
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.3 }}
              >
                <span className="text-primary text-[10px] uppercase tracking-[0.5em] font-bold block mb-6">{project.category}</span>
                <h2 className="text-4xl md:text-6xl font-serif font-bold tracking-tighter mb-8">{project.name}</h2>
                
                <div className="flex flex-wrap gap-10 mb-12 py-8 border-y border-border/5">
                  <div className="flex flex-col gap-1">
                    <span className="text-[8px] uppercase tracking-widest text-muted-foreground font-bold">Área Útil</span>
                    <span className="text-xl font-serif">{project.size}</span>
                  </div>
                  <div className="flex flex-col gap-1">
                    <span className="text-[8px] uppercase tracking-widest text-muted-foreground font-bold">Quartos</span>
                    <span className="text-xl font-serif">{project.bedrooms}</span>
                  </div>
                  <div className="flex flex-col gap-1">
                    <span className="text-[8px] uppercase tracking-widest text-muted-foreground font-bold">Banheiros</span>
                    <span className="text-xl font-serif">{project.bathrooms}</span>
                  </div>
                </div>

                <div className="space-y-12 mb-16">
                  <div>
                    <h4 className="text-[10px] uppercase tracking-[0.3em] font-bold text-primary mb-6">Conceito Arquitetônico</h4>
                  ...
                  <div>
                    <h4 className="text-[10px] uppercase tracking-[0.3em] font-bold text-primary mb-6">Planta Baixa</h4>
                    <motion.div 
                      whileHover={{ scale: 1.02 }}
                      className="aspect-video bg-muted/20 border border-border/10 rounded-sm overflow-hidden relative cursor-zoom-in group"
                    >
                      <img src={project.floorPlanImage} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-opacity" />
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <div className="bg-background/80 backdrop-blur-md px-6 py-3 rounded-full border border-border/10 text-[10px] uppercase tracking-widest font-bold flex items-center gap-2">
                          <Maximize2 size={14} /> Ampliar Planta
                        </div>
                      </div>
                    </motion.div>
                  </div>
                ...
                <div className="p-12 bg-primary/5 rounded-sm border border-primary/10">
                  <h5 className="text-xl font-serif font-bold mb-4">Gostou deste modelo, mas faria diferente?</h5>
                  <p className="text-muted-foreground text-sm font-serif italic mb-8">
                    Personalize este projeto e transforme essa referência em um chalé pensado para você.
                  </p>
                  <button 
                    onClick={() => onCustomize(project.name)}
                    className="w-full bg-primary text-primary-foreground py-5 text-[10px] uppercase tracking-[0.2em] font-bold hover:bg-primary/90 transition-all rounded-sm shadow-xl flex items-center justify-center gap-3"
                  >
                    Quero personalizar este modelo <ArrowRight size={16} />
                  </button>
                </div>
              </motion.div>
            </div>
...
export const Catalog = () => {
  ...
  const categories = ["Todos", "A-Frame", "Contemporâneos", "Rústicos", "Premium"];
  
  const filteredProjects = activeCategory === "Todos" 
    ? CATALOG_DATA 
    : CATALOG_DATA.filter(p => p.category === activeCategory);

  const headerRef = useRef(null);
  const isHeaderInView = useInView(headerRef, { once: true, margin: "-100px" });

  return (
    <section id="catalogo" className="py-32 md:py-56 bg-background relative overflow-hidden">
      {/* Background Decor */}
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div ref={headerRef} className="text-center mb-32 max-w-4xl mx-auto">
          <motion.span 
            initial={{ opacity: 0, y: 20 }}
            animate={isHeaderInView ? { opacity: 1, y: 0 } : {}}
            transition={{ duration: 0.8 }}
            className="text-primary text-[10px] uppercase tracking-[0.5em] font-bold block mb-8"
          >
            Nossos Modelos
          </motion.span>
          ...
          <motion.h2 
            ...
            className="text-5xl md:text-8xl font-serif font-bold tracking-tighter leading-[0.9] mb-12"
          >
          ...
          <motion.p 
            ...
            className="text-muted-foreground text-xl md:text-2xl font-serif italic leading-relaxed"
          >
            Explore alguns dos nossos modelos e encontre o ponto de partida para o chalé que você sempre imaginou.
          </motion.p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-6 md:gap-12 mb-24">
          {categories.map((cat, idx) => (
            <motion.button
              ...
              className={cn(
                "relative text-[10px] uppercase tracking-[0.3em] font-bold py-4 px-2 transition-all duration-500",
        ...
        <motion.div 
          layout
          className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-16 md:gap-24"
        >
        ...
        {/* Final CTA */}
        <div className="mt-40 text-center border-t border-border/5 pt-32">
          <motion.div
            initial={{ opacity: 0, y: 40 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          ...
          >
            <h3 className="text-3xl md:text-5xl font-serif font-bold tracking-tight mb-6">Não encontrou o modelo ideal?</h3>
            <p className="text-muted-foreground text-lg md:text-xl font-serif italic mb-12">
              Então vamos criar algo que seja só seu. Cada terreno e cada sonho pedem uma resposta arquitetônica única.
            </p>
            <a 
              href="#orcamento" 
              className="inline-flex items-center gap-4 bg-primary text-primary-foreground px-12 py-6 text-[11px] font-bold uppercase tracking-[0.3em] hover:bg-primary/90 hover:-translate-y-2 transition-all rounded-sm shadow-2xl"
            >
              Criar meu chalé <ArrowRight size={18} />
            </a>
          </motion.div>
        </div>
      </div>
      ...
      <ProjectModal 
        ...
        onCustomize={(name) => {
          ...
          // We could use an event emitter or global state to pre-fill the form
          console.log(`Pre-filling form with: ${name}`);
        }}
      />
    </section>
  );
